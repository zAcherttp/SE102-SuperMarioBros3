...
void World::HandleInput(Keyboard::State* kbState,
	...
	if (!m_player || !m_player->IsActive() || m_isPaused)
		return;
	Mario* mario = dynamic_cast<Mario*>(m_player);
	if (!mario || mario->IsTransitioning())
		return;
...
std::vector<Entity*> World::GetEntities() { return m_entities; }
...
void World::RenderDebug(
	DirectX::PrimitiveBatch<DirectX::DX11::VertexPositionColor>
	* primitiveBatch) {
	...
	if (m_player) {
		m_player->GetCollisionComponent()->RenderDebug(primitiveBatch,
			Colors::Lime);
		Vector2 pos = m_player->GetPosition();
...
void World::TogglePause() { m_isPaused = !m_isPaused; }
...
void World::Load(SpriteSheet* spriteSheet) {
	...
	// Reset HUD
	// TODO: World saves mario state to Game, then after loading, set the
	// properties
...
void World::LoadEntities(const json& data, const json& anim,
...
Entity* World::CreateEntity(int entType, const json& data,
	SpriteSheet* spriteSheet) {
	...
	case ID_ENT_RED_PIRANHA_SPIT_FIRE: {
		...
		entity =
			new FirePiranha(position, Vector2(width, height), spriteSheet, color);
		break;
	}
	case ID_ENT_GREEN_TROOPA: {
		...
		entity = new GreenTroopas(position, Vector2(width, height), spriteSheet,
			hasWing);
		break;
	}
	...
	case ID_ENT_GROUND: {
		...
		entity = new Ground(position, Vector2(width, height), countX, countY,
			isSolid, spriteSheet);
		break;
	}
	case ID_ENT_BUSH: {
		...
		if (brushType == 0) {
			...
			entity = new Bush(position, 1, spriteSheet, brushType);
		break;
	}
	case ID_ENT_CLOUD: {
		...
		if (cloudType == 0) {
			...
			entity = new Cloud(position, 0, spriteSheet, cloudType);
		break;
	}
	case ID_ENT_SCREW_BLOCK: {
		...
		entity = new ScrewBlock(position, Vector2(width, height), countX, countY,
			isSolid, spriteSheet, depth, color, isFloating);
		break;
	}
	case ID_ENT_PIPE: {
		float width = data["width"];
	...
	case ID_ENT_SKY_PLATFORM: {
		...
		int countX = data["countX"];
		bool isSolid = data["solid"];
		entity = new SkyPlatform(position, Vector2(width, height), countX, isSolid,
			spriteSheet);
		break;
	}
	case ID_ENT_LUCKY_BLOCK: {
		...
		entity = new LuckyBlock(position, Vector2(width, height), isSolid,
			spriteSheet, isSpecial, isMimic, isMushroom);
		break;
	}
	case ID_ENT_BLACK_BACKGROUND: {
		...
		float depth = data["depth"];
		entity = new BlackBackground(position, Vector2(width, height), countX,
			countY, spriteSheet, bgtype, depth);
		break;
	}
	...
	case ID_ENT_GRASS_BLOCK: {
		...
		int countY = data["countY"];
		bool isSolid = data["solid"];
		entity = new Grass(position, Vector2(width, height), countX, countY,
			isSolid, spriteSheet);
		break;
	}
	case ID_ENT_SECRET_WALL: {
		...
		int countX = data["countX"];
		int countY = data["countY"];
		entity = new SecretWall(position, Vector2(width, height), countX, countY,
			spriteSheet);
		break;
	}
	...
	case ID_ENT_MOVING_PLATFORM: {
		...
		bool isSolid = data["solid"];
		int countX = data["countX"];
		entity = new FloatingPlatform(position, Vector2(width, height), countX,
	...
	if (dynamic_cast<Mario*>(entity)) {
		...
		Log(__FUNCTION__,
			"Failed to create entity of type: " + std::to_string(entType));
		delete entity;
		return nullptr;
	}
...
void World::LoadAnimationsForEntity(Entity* entity, int type,
	const json& anim) {
	auto typeIt =
		std::find_if(anim.begin(), anim.end(), [type](const json& item) {
		return item.contains("type") && item["type"] == type;
			});

	if (typeIt == anim.end() || !typeIt->contains("sprites")) {
		Log(__FUNCTION__,
			"No animation data found for entity type: " + std::to_string(type));
		return;
	}

	// Special debugging for Goomba
	// if (type == ID_ENT_GOOMBA) {
	//	Log(__FUNCTION__, "Found animation data for Goomba (type " +
	//std::to_string(type) + ")");
	//}
	...
	for (const auto& sequence : (*typeIt)["sprites"]) {
		...
		/*if (type == ID_ENT_GOOMBA) {
				Log(__FUNCTION__, "Loading Goomba animation ID: " +
		std::to_string(animId) + " Name: " + sequence["name"].get<std::string>() +
						" Frames: " + std::to_string(sequence["frames"].size()));
		}
		else {
				Log(__FUNCTION__, "Entity ID: " + std::to_string(type) + " Animation
		ID: " + std::to_string(animId) + " Frames: " +
		...
		for (const auto& frame : sequence["frames"]) {
			...
			if (frameNameCache.find(frameName) == frameNameCache.end()) {
				frameNameCache[frameName] =
					std::wstring(frameName.begin(), frameName.end());
			}
		...
		bool loop =
			sequence.contains("loop") ? sequence["loop"].get<bool>() : false;
		float timePerFrame = sequence.contains("timePerFrame")
			? sequence["timePerFrame"].get<float>()
			: 0.1f;
		bool useVelocityScaling = sequence.contains("useVelocityScaling")
			? sequence["useVelocityScaling"].get<bool>()
			: false;
		float minTimePerFrame = sequence.contains("minTimePerFrame")
			? sequence["minTimePerFrame"].get<float>()
			: 0.05f;
		float maxTimePerFrame = sequence.contains("maxTimePerFrame")
			? sequence["maxTimePerFrame"].get<float>()
			: 0.2f;
		float velocityScaleFactor =
			sequence.contains("velocityScaleFactor")
			? sequence["velocityScaleFactor"].get<float>()
			: 1.0f;

		// Log(__FUNCTION__, "Animation " + std::to_string(animId) + " -
		// timePerFrame: " + (sequence.contains("timePerFrame") ? "found" : "using
		// default")); Log(__FUNCTION__, "Animation " + std::to_string(animId) + " -
		// loop: " + (sequence.contains("loop") ? "found" : "using default"));
		// Log(__FUNCTION__, "Animation " + std::to_string(animId) + " -
		// useVelocityScaling: " + (sequence.contains("useVelocityScaling") ?
		// "found" : "using default")); Log(__FUNCTION__, "Animation " +
		// std::to_string(animId) + " - minTimePerFrame: " +
		// (sequence.contains("minTimePerFrame") ? "found" : "using default"));
		// Log(__FUNCTION__, "Animation " + std::to_string(animId) + " -
		// maxTimePerFrame: " + (sequence.contains("maxTimePerFrame") ? "found" :
		// "using default")); Log(__FUNCTION__, "Animation " +
		// std::to_string(animId) + " - velocityScaleFactor: " +
		// (sequence.contains("velocityScaleFactor") ? "found" : "using default"));

		entity->DefineAnimation(animId, frameNames, loop, timePerFrame,
			useVelocityScaling, minTimePerFrame,
			maxTimePerFrame, velocityScaleFactor);
	}
}

Entity* World::GetPlayer() { return m_player; }

XMVECTORF32 World::GetBackgroundColor() const { return m_background; }
...
World* World::GetInstance() { return s_instance; }