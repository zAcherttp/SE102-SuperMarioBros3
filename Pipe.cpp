...
Pipe::Pipe(Vector2 position, Vector2 size, int countX, int countY, bool isSolid, SpriteSheet* spriteSheet, bool hasHead, bool isBonus)
...
void Pipe::Render(DirectX::SpriteBatch* spriteBatch)
{
    ...
    if(m_hasHead)
        {
            m_animator->Draw(spriteBatch, ID_SPRITE_BONUS_PIPE_TOP_LEFT, pos + Vector2(0.0f, 16.0f * (m_tileYcount - 1)),0.0f); // Draw the head above the pipe
            m_animator->Draw(spriteBatch, ID_SPRITE_BONUS_PIPE_TOP_RIGHT, pos + Vector2(16.0f, 16.0f * (m_tileYcount - 1)), 0.0f); // Draw the head above the pipe
        }
        for(int j = 0; j < m_tileYcount - m_hasHead ? 1 : 0; j++)
        {
            m_animator->Draw(spriteBatch, ID_SPRITE_BONUS_PIPE_BOT_LEFT, pos + Vector2(0, j * tileSize.y), 0.0f);
            m_animator->Draw(spriteBatch, ID_SPRITE_BONUS_PIPE_BOT_RIGHT, pos + Vector2(16, j * tileSize.y), 0.0f);
        }
    }
