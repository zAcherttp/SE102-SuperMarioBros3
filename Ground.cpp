...
void Ground::Render(DirectX::SpriteBatch* spriteBatch)
{
	...
	{
        m_animator->Draw(spriteBatch, ID_SPRITE_INDEPENDENT_PLATFORM, pos, 1.0f);
        return; // exit early if it's a single tile
    }
    // draw top left corner of the tile, then the middle, then the top right corner
    m_animator->Draw(spriteBatch, ID_SPRITE_GROUND_TOP_LEFT, pos, 1.0f);
    for (int i = 1; i < m_tileXcount - 1; i++)
    {
        m_animator->Draw(spriteBatch, ID_SPRITE_GROUND_TOP_MID, pos + Vector2(i * tileSize.x, 0), 1.0f);
    }
    m_animator->Draw(spriteBatch, ID_SPRITE_GROUND_TOP_RIGHT, pos + Vector2((m_tileXcount - 1) * tileSize.x, 0), 1.0f);
    // draw the middle of the tile
    for (int j = 1; j < m_tileYcount; j++)
    {
        m_animator->Draw(spriteBatch, ID_SPRITE_GROUND_BOT_LEFT, pos + Vector2(0, j * tileSize.y), 1.0f);
        for (int i = 1; i < m_tileXcount - 1; i++)
        {
            m_animator->Draw(spriteBatch, ID_SPRITE_GROUND_BOT_MID, pos + Vector2(i * tileSize.x, j * tileSize.y), 1.0f);
        }
        m_animator->Draw(spriteBatch, ID_SPRITE_GROUND_BOT_RIGHT, pos + Vector2((m_tileXcount - 1) * tileSize.x, j * tileSize.y), 1.0f);
    }
}
