...
inline void Animator::LoadSpriteSheet(ID3D11ShaderResourceView* texture, const wchar_t* sheetDataFile)
{
    mSpriteSheet = std::make_unique<SpriteSheet>();
    mSpriteSheet->Load(texture, sheetDataFile);
}

inline void Animator::DefineAnimation(const std::string& name, const std::vector<const wchar_t*>& frameNames, bool loop, float baseTimePerFrame, bool useVelocityScaling, float minTimePerFrame, float maxTimePerFrame, float velocityScaleFactor)
{
...
inline void Animator::SetAnimation(const std::string& name, bool reset)
{
...
inline void Animator::Update(float elapsed, float velocity)
{
...
inline void Animator::Draw(DirectX::SpriteBatch* batch, const DirectX::XMFLOAT2& position)
{
...
inline void Animator::SetFlipHorizontal(bool flip)
{
    if (flip)
...
inline void Animator::SetFlipVertical(bool flip)
{
    if (flip)
...
inline bool Animator::IsFlippedHorizontally() const
{
    return (mEffects & DirectX::SpriteEffects_FlipHorizontally) != 0;
}

inline bool Animator::IsFlippedVertically() const
{
    return (mEffects & DirectX::SpriteEffects_FlipVertically) != 0;
}

// Allow direct access to sprite effects for advanced usage
inline void Animator::SetSpriteEffects(DirectX::SpriteEffects effects)
{
    mEffects = effects;
}

inline DirectX::SpriteEffects Animator::GetSpriteEffects() const
{
    return mEffects;
}

// Direction helper - common use case for side-scrollers
inline void Animator::SetDirection(int direction)
{
...
inline void Animator::Play() { mPaused = false; }

inline void Animator::Pause() { mPaused = true; }

inline void Animator::SetScale(float scale) { mScale = { scale, scale }; }

inline void Animator::SetScale(float scaleX, float scaleY) { mScale = { scaleX, scaleY }; }

inline void Animator::SetRotation(float rotation) { mRotation = rotation; }

// Status methods
inline bool Animator::IsPaused() const { return mPaused; }

inline std::string Animator::GetCurrentAnimation() const { return mCurrentSequence; }

inline int Animator::GetCurrentFrame() const { return mCurrentFrame; }
...
inline const std::map<std::string, AnimationSequence>& Animator::GetAnimations() const
{
    return mAnimations;
}

inline bool Animator::HasAnimation(const std::string& name) const
{
